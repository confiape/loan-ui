...
import { MultiSelectComponent, MultiSelectItem } from '../../ui/multiselect/multiselect';
import { FormFieldMetadata, SelectOption } from '../../../core/models/form-metadata';

/**
 * Generic form component that generates form fields based on metadata
 * Supports: text, number, email, password, textarea, select, multiselect, date, checkbox, radio
 *
 * @example
 * ```html
 ...
 *   (save)="service.onFormSave()"
 *   (cancel)="service.onFormCancel()"
 * />
 * ```
 */
@Component({
...
export class GenericCrudFormComponent implements OnInit {
  ...
  /**
   * Load all options for select/multiselect fields
   */
  ...
  /**
   * Transform SelectOption[] to MultiSelectItem[]
   */
  toMultiSelectItems(options: SelectOption[]): MultiSelectItem[] {
